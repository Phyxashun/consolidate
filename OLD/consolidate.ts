...
const JOB_DEFINITIONS: JobDefinition[] = [
    ...
    {
        name: 'STYLE_FILES',
        description: 'Style Files (css, scss, etc.)',
    ...
    const createJobsForExtension = (
        ...
        JOB_DEFINITIONS.map((def, index) => ({
            ...
            outputFile: path.join(
                outputDir,
                `${index + 1}_ALL_${def.name}.${fileExtension}`,
            ),
        }));

    return [
        ...createJobsForExtension(TS_OUTPUT_PATH, 'ts'),
        ...createJobsForExtension(TEXT_OUTPUT_PATH, 'txt'),
    ];
};
...
const ui = {
    title: () => {
        p.intro(
            `${pc.bgMagenta(pc.black('  󰽜  󰕘 CONSOLIDATE '))}  ${pc.dim('Project File Consolidator')}`,
        );
        p.log.message('');
    },
    ...
    noFiles: () =>
        p.outro(
            pc.yellow(
                'No matching files found for any job — nothing to consolidate.',
            ),
        ),
};
...
const consolidateJobs = {
    process: async (
        ...
        const sourceFiles = await fileSystem.findFiles(job.include, [
            ...(job.exclude || []),
            ...ignorePatterns,
        ]);
        ...
        await p.tasks(
            jobs.map(job => ({
                ...
                task: async (
                    ...
                    const fileCount = await consolidateJobs.process(
                        job,
                        ignoreList,
                        message,
                    );
...
p.updateSettings({
    messages: { cancel: 'Operation cancelled', error: 'An error occurred' },
});

const getUserInput = async (message: string): Promise<boolean> => {
    const action = await p.confirm({ message, active: 'Yes', inactive: 'No' });
...
const convertToPdf = async (): Promise<void> => {
    ...
    try {
        ...
        const txtPath = path.resolve(
            process.cwd(),
            './ALL/txt/1_ALL_SOURCE_FILES.txt',
        );
        await sleep(400);

        let suffix = generateShortId();
        let outputFile = `output-${suffix}.pdf`;
        let outputPath = path.resolve(process.cwd(), `./ALL/pdf/${outputFile}`);

        await mkdir(path.resolve(process.cwd(), './ALL/pdf'), {
            recursive: true,
        });
        ...
        while (await Bun.file(outputPath).exists()) {
            ...
            outputPath = path.resolve(process.cwd(), `./ALL/pdf/${outputFile}`);
        }
...
const main = async (): Promise<void> => {
    ...
    const fullIgnoreList = [
        ...new Set([...IGNORE_LIST_BASE, ...gitignorePatterns]),
    ];
